...
export function ProjectAttachmentTable({
  attachments,
  projects,
  onDuplicate,
  onMove,
  ...
  return (
    <>
      {selectedIds.length > 0 && (
        <SelectionToolbar
          label={t("hand.attachmentsSelected").replace(
            "{count}",
        ...
        >
          ...
          <Button variant="ghost" onClick={() => setMovingIds(selectedIds)}>
            {t("hand.moveAttachment")}
          </Button>
          <Button
            variant="danger"
            onClick={() => {
        ...
        <table className="w-full min-w-[760px] text-left text-sm">
          <thead className="border-b border-zinc-200 bg-zinc-50/80 text-[11px] font-semibold uppercase tracking-wide text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900/70">
            <tr>
              ...
              <th className="px-4 py-3">{t("hand.fileSize")}</th>
            ...
            {attachments.map((item) => (
              ...
              >
                <td className="px-4 py-4">
                  <span className="relative grid size-9 items-center justify-center">
                    <span onClick={(event) => event.stopPropagation()}>
                      <Checkbox
                        aria-label={item.name}
                        checked={selectedIds.includes(item.id)}
                        className={`absolute inset-0 m-auto ${selectedIds.length ? "opacity-100" : "pointer-events-none border-zinc-300 bg-zinc-100 text-zinc-400 opacity-0 transition-opacity dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-500 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100"}`}
                ...
                <td className="px-4 py-4">
                  <Badge>{fileType(item)}</Badge>
                </td>
                <td className="whitespace-nowrap px-4 py-3 text-zinc-500">
                  {formatNumber(item.size / 1024, { maximumFractionDigits: 1 })}{" "}
                  KB
                </td>
                <td className="whitespace-nowrap px-4 py-3 text-zinc-500">
      ...
      {menu && (
        <ContextMenu position={menu.position} onClose={() => setMenu(null)}>
          <ContextMenuItem
            onClick={() => {
              void openInFinder(menu.attachment, "open");
              setMenu(null);
            }}
          >
          ...
          <ContextMenuItem
            onClick={() => {
              void openInFinder(menu.attachment, "reveal");
              setMenu(null);
            }}
          >
            <FolderOpenIcon className="size-4" />
            {t("hand.openFileLocation")}
          </ContextMenuItem>
          <ContextMenuItem
            onClick={() => {
              onDuplicate(actOn(menu.attachment));
              setMenu(null);
            }}
          >
          ...
          <ContextMenuItem
            onClick={() => {
              setMovingIds(actOn(menu.attachment));
              setMenu(null);
            }}
          >
            <FolderOpenIcon className="size-4" />
          ...
          <ContextMenuItem
            danger
            onClick={() => {
              const ids = actOn(menu.attachment);
              setMenu(null);
              if (window.confirm(t("common.confirmDelete"))) {
                onDelete(ids);
                setSelectedIds([]);
  ...
  return (
    ...
    >
      ...
      {attachment.mimeType.startsWith("video/") && (
        <video src={src} controls className="max-h-[70vh] w-full" />
      )}
      ...
      {!attachment.mimeType.startsWith("image/") &&
        !attachment.mimeType.startsWith("video/") &&
        !attachment.mimeType.startsWith("audio/") &&
        !["pdf", "md", "markdown", "csv", "txt"].includes(extension ?? "") && (
          <p className="text-sm text-zinc-500">
            {t("hand.previewUnavailable")}
          </p>
        )}
    </Dialog>
  );
  ...
  return (
    ...
    >
      <div className="grid gap-4">
        ...
        >
          <option value="">{t("hand.selectProject")}</option>
        ...
        <Button disabled={!projectId} onClick={() => onChoose(projectId)}>
          {t("hand.moveAttachment")}
        </Button>
      </div>
    </Dialog>
