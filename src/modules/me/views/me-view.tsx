...
import { PageHeader, SectionHeader, StatCard } from "@/shared/components/page-elements";
...
export function MeView() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader eyebrow={t("me.eyebrow")} title={t("me.profileTitle")}
        subtitle={t("me.profileSubtitle")} />
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label={t("dashboard.totalTime")}
          value={`${formatNumber(vm.metrics.totalMinutes / 60, { maximumFractionDigits: 1 })} ${t("common.hours")}`} />
        <StatCard label={t("dashboard.effectiveRate")} value={rate(vm.metrics.effectiveRate)}
          hint={t("dashboard.metricEffective")} />
        <StatCard label={t("dashboard.completionRate")} value={rate(vm.metrics.completionRate)}
