  ...
  return (
    <article>
      <div className="flex items-center gap-1 border-b border-zinc-200 p-3 dark:border-zinc-800">
        ...
        <IconButton label={t("messages.reply")} onClick={onReply}>
          <ArrowUturnLeftIcon className="size-5" />
        </IconButton>
        <IconButton label={t("messages.forward")} onClick={onForward}>
          <ArrowUturnRightIcon className="size-5" />
        </IconButton>
        <IconButton
          label={t("messages.markUnread")}
          disabled={!message.readAt}
      ...
      <div className="p-6">
        ...
        {!!message.references.length && (
          <section className="mt-5 border-t border-zinc-200 pt-5 dark:border-zinc-800">
            <h3 className="mb-3 text-sm font-semibold">
              {t("messages.references")}
            </h3>
            <div className="flex flex-wrap gap-2">
              {message.references.map((item) => (
                <Badge key={`${item.kind}-${item.id}`}>{item.label}</Badge>
              ))}
            </div>
          </section>
        )}
