...
type Context = { params: Promise<{ noteId: string; image: string }> };
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const directory = getStoragePath("notes", noteId);
    ...
    return Response.json({
      image,
      url: `/api/notes/${encodeURIComponent(noteId)}/${image}`,
    });
  } catch (error) {
    return Response.json(
...
export async function GET(_request: Request, context: Context) {
  const { noteId, image } = await context.params;
  if (!identifier.test(noteId) || !imageIdentifier.test(image))
    return Response.json(
      { error: "Invalid image identifier." },
      { status: 422 },
    );
  try {
    const file = await fs.readFile(getStoragePath("notes", noteId, image));
